///movement_and_collision(direction, movespeed, wall object)
...
if(!place_meeting_round(x+lengthdir_x(_wcr,_movement_direction),y+lengthdir_y(_wcr,_movement_direction),_wall))
{   //We are not against a wall.  Move.
    ...
    while(place_meeting_round(x,y,_wall))
...
{   //Check for ramps!  Now things are starting to get crazy.
    ...
    var _angle=45;
    var _ramp=false;
    if(!place_meeting_round(x+lengthdir_x(_wcr,_movement_direction+_angle),y+lengthdir_y(_wcr,_movement_direction+_angle),_wall))
    {   //Found a ramp counter clockwise!
        _movement_direction+=_angle;
        _ramp=true;
    }
    else if(!place_meeting_round(x+lengthdir_x(_wcr,_movement_direction-_angle),y+lengthdir_y(_wcr,_movement_direction-_angle),_wall))
    {   //Then clockwise
        _movement_direction-=_angle;
        _ramp=true;
    }
